package com.sn.snmedia.config;

import com.sn.snmedia.security.JwtAuthConverter;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, JwtAuthConverter jwtAuthConverter) throws Exception {
        http
                .authorizeHttpRequests(auth -> auth
                        // Дозволяємо GET-запити на скачування файлів (публічний доступ)
                        .requestMatchers(HttpMethod.GET, "/media/download/**").permitAll()
        ...
        return new WebMvcConfigurer() {
            ...
            public void addCorsMappings(CorsRegistry registry) {
                registry.addMapping("/**")           // все пути
                        .allowedOrigins("http://localhost:5173") // звідки дозволяємо
                        .allowedHeaders("*")
                        .exposedHeaders("Authorization", "Content-Disposition") // Додаємо Content-Disposition
    ...
    // Бін JwtAuthConverter потрібен для SecurityConfig
    @Bean
